---
name: setup-project
description: Configure or reconfigure a local Hive harness only for an explicitly identified project, repository, folder, or path. Route global user-scope setup, preference changes, and bare Hive setup to configure.
---
...
- Do not select it for global or user-scope preference changes, or for a bare request to set up,
  install, configure, or reconfigure Hive. Those requests belong to `configure`.
- When the user explicitly requests both scopes, let `configure` finish first and wait for a
...
3. Ask one setup question at a time.
   ...
   - When `auto-setup-project` supplies a schema-valid evidence record, accept only
...
2. **Project kind**
   ...
   - `auto-setup-project` may infer `general` only from an explicit canonical project purpose;
