...
8. `status` is inspection only and never substitutes for an automatic-dispatch preflight. After mutation, run `enforce` only for pending automatic dispatch or the required same-session threshold recheck in the control reference. Treat `session_override=absent` or `stale` as enabled. Never copy an override to another host, session, or process.
