...
3. 표시된 선택지 중 하나 수용
   ...
   - `manual`: 이름·양쪽 exact model/effort·범위·권한·trigger를 명시한 request 작성
   - `revise`: 변경 field만 반영한 request 재작성

4. request의 `decision_digest`와 정확히 일치하는 동의 뒤 생성

   ```text
   hive agent create --request <creation.json> --root <user-root|project-root> --accept-decision-digest <sha256:...> --output json
   ```
