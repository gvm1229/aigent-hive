...
3. For automatic preparation, use this exact sequence. `$aigent-hive:manage-usage` alone is not a
   dispatch authorization:

   1. Run `$aigent-hive:manage-usage` for the current host session and stop unless it returns `allowed`.
...
- Reuse `$aigent-hive:save-progress`, `$aigent-hive:resume-work`, `$aigent-hive:handoff-role`, and
  `$aigent-hive:verify-package` for their narrow data contracts.
