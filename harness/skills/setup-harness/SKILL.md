---
name: setup-harness
description: Inspect a project and configure its local Aigent Hive harness through the signed Hive CLI. Use when a user asks to set up, initialize, install, reconfigure, or regenerate Aigent Hive; when creating AGENTS.md and scoped directives from a project profile; or when selecting a host, orchestration layer, persistent roles, memory policy, usage guard, and manually approved optional Skills.
---
...
4. Resolve compatibility.
   - Allow OMX only with Codex and OMC only with Claude Code.
   - Never install, update, configure, or invoke OMX/OMC.
   - Treat the user's selection as authoritative. If requested, probe only the public executable path and `--version`; never infer state by reading runtime directories.
   - Use `host-native` when no external layer is selected; do not install Hive copies of plan, Ralph, or team.
...
6. Run a dry run and validate the write set.
