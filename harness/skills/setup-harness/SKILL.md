...
6. Offer fallback hooks only after conclusive absence.
   ...
   - For `absent`, show each exact capability, event, `.hive/hooks/<capability>` path, `hive hook --capability <capability> --event <event> --output json` command, and content digest.
   ...
   - Hooks never classify `UserPromptSubmit`, rewrite prompts, activate Skills, ingest memory, spawn subagents, orchestrate work, or decide continuation.
