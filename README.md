...
[![Source license](https://img.shields.io/badge/source-GPL--3.0--only-blue)](LICENSE)
...
├── LICENSE              # GitHub가 감지하는 primary GPL-3.0-only 전문
...
> **Open source must stay open source.**

Hive 자체의 공개 source를 기반으로 배포되는 개선판도 계속 공개 source로 남아야 한다는 원칙으로 CLI/source에는 GPLv3를 적용한다. 반면 출하 harness는 사용자의 프로젝트에 들어가는 material이므로 Apache-2.0으로 분리해 그 프로젝트의 라이선스 선택을 침범하지 않는다.

- **CLI와 source — [`GPL-3.0-only`](LICENSE):** `harness/`를 제외한 저장소 source에 적용된다. GPL-covered 수정판을 배포할 때는 대응 source와 같은 라이선스를 제공해야 하지만, 개인·사내에서만 수정해 사용하는 경우 공개 의무는 없다.
