...
- **CLI와 source — [`GPL-3.0-only`](LICENSES/GPL-3.0-only.txt):** `harness/`를 제외한 저장소 source에 적용된다. GPL-covered 수정판을 배포할 때는 대응 source와 같은 라이선스를 제공해야 하지만, 개인·사내에서만 수정해 사용하는 경우 공개 의무는 없다.
