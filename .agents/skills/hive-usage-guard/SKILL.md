...
2. While `gate` exits `10` or `11`, block every ordinary task in the same session. The only
   permitted work is guard status, threshold, enable, disable, toggle, or consented fallback
   install control and the minimal reply that explains the block.
...
12. When CodexBar is used, prefer its primary session window and use the weekly window only when
   the primary window is absent. Treat missing or unqualified CodexBar as `usage_unknown`.
13. When CodexBar is missing, return the provider-specific notification and exact `--dry-run`
   preview. Decline leaves core guard controls usable and automatic dispatch unknown.
