...
summary: "Public and live recovery checks with bounded adherence and cost evidence."
...
Test.9 restores approved Markdown within 4,096 bytes, with path hints and file guards. Restore and child events avoid parent deduplication and added model calls. Changed text is not promoted; removal stays available. Public binaries passed 29 hook contracts per OS. Windows Codex with gpt-6-astra: three conditions passed 5/5; three resumes and children verified. Added input fell 2,239→526 tokens (76.5%); command p95 grew 0.10%. Budget failures remain failed; offline product checks passed. Maintainer accepts this test overrun as nonblocking for 0.11.0, with real-use observation for measurement differences; no usage-guard or future budget waiver. Samples do not guarantee universal adherence or billing savings.
