...
- 안정판 기준·npm `latest`: `0.10.0`, 정식 배포일 2026-09-02
- GitHub 정식 Release: `v0.10.0`, source `301147fab8252954b29b7393327dfcff18eb8b1`
- 안정판 공개 문서: `0.10.0` 안내와 시험판 비노출 gate 적용. 공개 문서는 사용자에게 안정판만 안내
...
- Agent 소유: 0건. `0.10.0` 정식 출시 checklist 완료
- branch 규칙: 모든 변경 branch → develop → main. main PR head=develop, 안정판 문서 PR도 develop 선행
- 유지보수자 권한: `0.10.0` 안정판 명시 승인을 수령·사용 완료. 이후 안정판은 새 버전별 명시 승인이 필요
- 기술 차단: 0건. `test.13` 후보 `33545448836`·게시 `33546575448`·최종 세 운영체제 수용 `33546986588`과 stable candidate `33549229092`·publication `33550812035` 성공
- 구독자 안내: 문구 승인 뒤 외부 지문 자동 등록. 현재 승인된 `0.10.0` 지문 등록 완료, 출시마다 GitHub 수동 설정 불필요
- 등록 도구·전송 회귀: Windows 21개 통과. 실제 등록 성공·항목 존재 확인, GitHub 비밀 값 재조회 불가. 정식 게시 workflow가 승인된 안내문을 사용
...
- 안정판 `REL10-005–007`: `0.10.0`에서 완료. 이후 안정판은 새 버전명 포함 사용자 승인 전 `main` 통합·태그·게시·설치 금지
