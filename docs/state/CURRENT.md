...
- plan revision: `225`
- 현재 milestone: `HBC95-001`–`HBC95-005` historical project-base registry·migration declaration parity·local matrix, `AUP95-001`–`AUP95-006` authenticated saved multi-host scope·bare binary update 뒤 user projection refresh, `RQC95-001`–`RQC95-007` declared compatibility contract·compiled/package artifact local gate 완료. Windows local full gate·문서 Wiki 검증 완료. `MAC95-001` macOS external qualification은 유지보수자 수행 대기. 공개 시험판·안정판 게시는 명시 제외. `0.10.0` 후보 작업 제외 유지
...
- `0.9.5` local execution graph: `docs/plans/active/v0.9.5-release-loop.graph.md`, run ID `v095-local-qualification`. 사용자 명시 경계에 따라 `HBC95-006`·`AUP95-007`·`REL95-001–006`의 public test·protected `main` 통합·stable publication·현재 Windows stable install 수행 금지. 해당 배포 프로토콜은 `docs/plans/active/release-0.9.5-stable-publication.md`에 보류 기록
