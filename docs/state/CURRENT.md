...
- plan revision: `241`
- 현재 milestone: `HBC95-001`–`HBC95-005`, `AUP95-001`–`AUP95-006`, `RQC95-001`–`RQC95-007`, `NUS-029–030`, `KBP95-001–003`, `REL95-001–004`, `DUP95-002` 완료. M2 MacBook Air public `test.14 → test.15` direct install은 dedicated root `public-hive-acceptance-51180335c8824f98a673cf764b38f4e6`에서 setup·install·validate·update-check·update·final validate와 최초·최종 version 기록 성공. Windows `test.12 → test.13` actual host acceptance와 PortareFolium `0.9.2 → test.15` public project-upgrade matrix도 성공. `develop` CI `32110526256` Linux full·Windows/macOS smoke·전체 적합성·protected gate 성공. 다음은 `REL95-005` protected `main` integration이며, 안정판 npm publication은 유지보수자 별도 go sign 전 금지. `0.10.0` 후보 작업 제외 유지
...
- 확인된 release qualification gap: `0.9.4-test.1`은 `0.9.3 → 0.9.4-test.1` user install·validate 수용, `0.9.2` consumer project lifecycle 부재. migration table source range와 embedded full-base registry·compiled/package artifact matrix의 automatic cross-check 부재. `RQC95`: declared compatibility contract의 executable coverage·digest-bound public test evidence gate. 정본: `docs/plans/active/release-compatibility-qualification-0.9.5.md`
- `0.9.5` execution order: `v095-local-qualification` macOS source qualification·native sensor recovery·knowledge bundle acceptance → `v095-stable-release` numbered public test·Windows acceptance·protected `main` integration·same-byte stable publication. `0.9.5-test.3` candidate `31980927136`·publication `31981450374`·GitHub prerelease·npm six-package `test` 확인 완료. public direct installer와 npm `0.9.4 → test.3` isolated install 성공. existing Codex marketplace ownership·user installation manifest mismatch로 user projection validate·bare update와 public `0.9.2` project setup은 mutation 없이 중단. Current-session guard는 유지보수자 명시 승인으로만 일시 해제, global 20% safeguard 유지. CodexBar 미사용
- PortareFolium qualification plan: actual `0.9.2` consumer의 48개 full-ledger Hive projection과 required support config만 fresh `tests/work` fixture에 copy. project source·credential-adjacent·runtime·index·knowledge·host-global byte 제외. public `0.9.5-test.3` binary의 project-local `scan`·`dry-run`·`apply`·`validate`와 local/foreign preservation evidence로 `REL95-004` 수용 예정. 이 경로는 Codex plugin marketplace 미사용
- `REL95-003` plan: Windows 11 Home·M2 MacBook Air의 dedicated test root에서 `0.9.5-test.8` authenticated baseline 후 `hive update --check` 무변경, explicit `hive update --channel test`의 `test.8 → test.9` package-owner activation, new executable user projection refresh·validate, foreign host configuration 보존 확인. test runner 밖 root 참조는 mutation 전 실패. `REL95-004`: `test.4` public artifact 수용 완료. `test.5` direct installer optional signer fallback marker 오판과 `test.7` Windows 실행 파일 잠금 거부 뒤 `DUP95-001` 보강
- `test.6` recovery: candidate `32047934686` 자산 SHA-256 확인 뒤 GitHub prerelease 25 assets 수동 복구, npm 여섯 package `test=0.9.5-test.6` 유지. `test.7`: candidate `32051985762`, publication `32052974383`, GitHub prerelease·npm 여섯 package `test=0.9.5-test.7` 확인
- local correction evidence: `32bf5df` historical `0.9.2` marker의 `wiki_backend` render 누락과 first-apply override 수렴 결함 보정. current Windows의 PortareFolium read-only source copy 57개 Hive input에서 `scan`·`dry-run`·`apply`·`validate`, local marker·foreign sentinel 보존, tampered ledger no-mutation 성공. 원본 project mutation `0건`
- isolated Codex evidence: current Windows의 dedicated test root에서 public `0.9.5-test.3` direct binary로 user setup·Codex install·validate 성공. Codex process는 test root configuration만 사용. test3에는 explicit test-channel activation이 없어 `REL95-003` completion 증거 아님
- pending candidate: `5f3bb93` explicit test-channel activation과 `45a9af8` public acceptance runner를 포함한 `0.9.5-test.4`, 뒤이은 `test.4 → test.5` actual user projection refresh 수용
- native sensor recovery evidence: invalid supplied Codex account digest는 complete authenticated native account 하나가 있을 때 unbound native 재측정 1회로 자동 복구. identity 누락·복수·malformed·stale·limited는 fail-closed, CodexBar 호출 없음. focused Rust 2건·usage suite 39건·strict Clippy 및 현재 macOS actual sensor `usage check` 통과
- knowledge-bundle evidence: macOS 격리 사용자 루트에서 `.hivekb` global export·SHA-256 `b852d5576ea74f2d63ab7f3a453e3e2a9ffa24b8b351df8b377e71e09ff68da9` 확인·dry-run·apply·destination lint 성공. 번들은 canonical Markdown 두 항목만 포함, SQLite·runtime·project-private 항목 0건
- macOS `MAC95-001` evidence: macOS 26.5.2 arm64와 Rosetta x86_64에서 source `7ad1e58` product bytes의 locked release build·format·workspace test·archive direct install·fake Antigravity user install dry-run/apply/validate·compiled CLI historical project upgrade `scan`/`dry-run`/rollback/`apply`/`validate` 통과. arm64 binary/archive SHA-256 `d968125f11c875f6df1d48a3d394902808f334a8145ab95441aa389eb1cfdf0d`/`a30d63a4714089a27c8b4fa93a44411ee5dfc3b5bd33d5e078f30f50fbdea2da`, x86_64 binary/archive SHA-256 `c3ea769a7ca6cc4d7905e0e161bbd8ec0d92df2467bc3bfac73f639727128679`/`33db1b38e4859e79a228cbcfd80188afb15afb7d94ee0f425ba6220684841ba1`. 두 Mach-O는 ad-hoc signature, TeamIdentifier 없음. Windows unsigned disclosure는 별도 release contract 유지
- local implementation evidence: `0.9.4` exact frozen project base와 compiled public-surface baseline 추가, `0.9.5` cumulative inventory의 `judge-attestation.schema.json` 보존. compiled CLI `0.9.1`·`0.9.2`·`0.9.3`·`0.9.4` project upgrade matrix의 `scan`·`dry-run`·injected rollback·`apply`·`validate`, signed release update dry-run·apply, tampered base no-mutation, foreign new directive collision, directive rollback 통과. `check-project-base-coverage.py`는 full base 이전 same-major source range를 실패 폐쇄하고 candidate artifact에 다이제스트 결합 수용 보고서 연결. bare update authenticated saved projection scope·new executable apply/validate matrix 통과. local npm pack·global install 3건, Phase 6 direct installer contract 19건 통과와 macOS-only 8건 건너뜀. Fresh `cargo fmt --check`·strict Clippy·workspace test 통과; Windows 권한·운영체제 한정 skip은 별도 유지
