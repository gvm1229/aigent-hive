...
- plan revision: `233`
- 현재 milestone: `HBC95-001`–`HBC95-005`, `AUP95-001`–`AUP95-006`, `RQC95-001`–`RQC95-007`, `NUS-029–030`, `KBP95-001–003`, `REL95-001–002` 완료. `REL95-004`: PortareFolium `0.9.2` Hive-owned subset fixture와 `0.9.5-test.4` 수용. `REL95-003`: Windows 11 Home·M2 MacBook Air 전용 시험 루트에서 `test.4 → test.5` explicit test-channel package-owner activation·user projection refresh·validate 수용. `REL95-005–006` 수용 뒤 순서 유지. `0.10.0` 후보 작업 제외 유지
...
- `REL95-003` plan: Windows 11 Home·M2 MacBook Air의 dedicated test root에서 `0.9.5-test.4` authenticated baseline 후 `hive update --check` 무변경, explicit `hive update --channel test`의 `test.4 → test.5` package-owner activation, new executable user projection refresh·validate, foreign host configuration 보존 확인. test runner 밖 root 참조는 mutation 전 실패. `REL95-004`는 `test.4` public artifact, `REL95-003`은 `test.5` public artifact 수용
