...
- 기준 branch: `main` stable `8b37323`, `develop` release compatibility correction `0fd5ea8`
...
- `0.9.5` execution order: `v095-local-qualification` macOS source qualification·native sensor recovery·knowledge bundle acceptance → `v095-stable-release` numbered public test·Windows acceptance·protected `main` integration·same-byte stable publication. `0.9.5-test.2` 뒤 `2208aa4` 수용 규약 보강과 `0fd5ea8` historical full-base release update·surface baseline correction으로 새 번호 `0.9.5-test.3` 수용 필요. Current-session guard는 유지보수자 명시 승인으로만 일시 해제, global 20% safeguard 유지. CodexBar 미사용
