...
- plan revision: `223`
- 현재 milestone: `HBC95-001`–`HBC95-006` historical project-base registry·migration declaration parity·release matrix, `AUP95-001`–`AUP95-007` authenticated saved multi-host scope·bare binary update 뒤 user projection refresh, `REL95-001`–`REL95-006` 번호 public test·Windows x64 수용·protected `main` 통합·stable publication·현재 Windows 안정판 설치 계획. `0.10.0` 후보 작업 제외 유지
...
- `0.9.5` release execution graph: `docs/plans/active/v0.9.5-release-loop.graph.md`, run ID `v095-stable-release`. Public test 수용 전 stable publication·현재 Windows stable install 실행 금지
