...
- 기준 branch: `main` stable `8b37323`, `develop` release metadata `7ad1e58`
...
- local implementation evidence: `0.9.4` exact frozen project base 추가, compiled CLI `0.9.1`·`0.9.2`·`0.9.3` project upgrade matrix의 `scan`·`dry-run`·injected rollback·`apply`·`validate`, tampered base no-mutation 통과. `check-project-base-coverage.py`는 full base 이전 same-major source range를 실패 폐쇄하고 candidate artifact에 다이제스트 결합 수용 보고서 연결. bare update authenticated saved projection scope·new executable apply/validate matrix 통과. local npm pack·global install 3건, Phase 6 direct installer contract 19건 통과와 macOS-only 8건 건너뜀. 최종 Windows: Rust workspace·strict Clippy·문서 40건·security 103건·contract 362건·integration 60건·release 53건 통과; Windows 권한·운영체제 한정 skip은 별도 유지
