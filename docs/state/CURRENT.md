...
- plan revision: `231`
- 현재 milestone: `HBC95-001`–`HBC95-005`, `AUP95-001`–`AUP95-006`, `RQC95-001`–`RQC95-007`, `NUS-029–030`, `KBP95-001–003`, `REL95-001–002` 완료. `REL95-004`: PortareFolium `0.9.2` Hive-owned subset fixture 수용. `REL95-003`: clean Codex profile에서 package-owner activation·user projection refresh·validate 수용. independent golden fixture regression 보강 뒤 `0.9.5-test.4` 발행과 두 수용 재실행. `REL95-005–006` 수용 뒤 순서 유지. `0.10.0` 후보 작업 제외 유지
...
- `REL95-003` plan: clean disposable Codex profile에서 public `0.9.4` authenticated baseline 후 `hive update --check` 무변경, exact test target package-owner activation, new executable user projection refresh·validate, foreign host configuration 보존 확인. test target discovery 불가 또는 activation failure는 manual npm replacement 없이 next public test repair 기준. PortareFolium `REL95-004`와 golden fixture regression source change 뒤 `0.9.5-test.4` 두 수용 재실행
