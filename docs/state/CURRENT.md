...
- plan revision: `235`
- 현재 milestone: `HBC95-001`–`HBC95-005`, `AUP95-001`–`AUP95-006`, `RQC95-001`–`RQC95-007`, `NUS-029–030`, `KBP95-001–003`, `REL95-001–002`, `REL95-004` 완료. `REL95-003`: Windows 11 Home·M2 MacBook Air 전용 시험 루트에서 `test.6 → test.7` explicit test-channel package-owner activation·user projection refresh·validate 수용. `REL95-005–006` 수용 뒤 순서 유지. `0.10.0` 후보 작업 제외 유지
...
- `REL95-003` plan: Windows 11 Home·M2 MacBook Air의 dedicated test root에서 `0.9.5-test.6` authenticated baseline 후 `hive update --check` 무변경, explicit `hive update --channel test`의 `test.6 → test.7` package-owner activation, new executable user projection refresh·validate, foreign host configuration 보존 확인. test runner 밖 root 참조는 mutation 전 실패. `REL95-004`: `test.4` public artifact 수용 완료. `test.5` direct installer optional signer fallback marker 오판은 user projection 전 안전 중단, `b8e4c79` 뒤 `test.6 → test.7` 재수용
- external publication block: `test.6` candidate `32047934686` 성공. publication `32048885517`·`32049162013`·`32049404951`은 candidate artifact API HTTP 503, `32049404951`은 npm 여섯 package·annotated tag 뒤 GitHub Release API HTTP 503. package `test=0.9.5-test.6`, GitHub prerelease asset 미완성. recovery: GitHub artifact·Release API 회복 뒤 exact candidate assets로 prerelease 생성
