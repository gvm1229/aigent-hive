...
라이선스 경계도 같은 분리를 따른다. Hive source의 기본 라이선스는 `Apache-2.0`지만 `harness/**`와 그로부터 생성된 Aigent Hive 소유 material은 `Apache-2.0`이다. 생성된 harness는 `.hive/` 안에 자체 라이선스 전문을 두며 소비자 프로젝트 root의 license를 변경하지 않는다.
