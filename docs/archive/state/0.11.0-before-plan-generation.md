...
- 계획 ID 21개를 [기존 출시 형식으로 정합화](../plans/refactor-id-mapping.md). 등록 범위 밖 완료 ID 수집은 공통 파서로 수정 예정, 제품 완료 처리 없음
...
- 현재 요청: `0.11.0` 전체 구현 계획 실행, 안정판 및 실제 사용자 설치의 기존 권한 경계 유지
...
- 현재 첫 작업: `RFB-001–002` 확정 버전의 제품 메타데이터·호환성·기준 측정, 이후 `RFS-001–002` 상태 검사 기반
- `RB104-004`: 기존 소유 문서와 미완료 상태 유지; 다른 호스트의 실제 근거 없이 완료 처리 금지
- 검증 범위: Windows Codex의 문서·코드 조사. 제품 시험·새 대화 수용·다른 운영체제 실행은 후속 구현 범위
- 계획 검증: Markdown 링크·항목 집계·문서 크기 검사 성공. 전체 문서 시험은 기존 Source Wiki 원본 지문 불일치로 실패; 기존 사용량 보호 안내의 문체 오류 5개·Skill 투영 불일치는 `RFB-002`의 기준 정리 대상
