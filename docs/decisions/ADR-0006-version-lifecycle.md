...
Aigent Hive source, release bundle과 installed harness는 `X.Y.Z` product version을 공유한다. 현재 backward-compatible Phase 4 persistent role, durable run과 fresh-session recovery milestone은 `0.5.0`이며 root `Cargo.toml`의 `workspace.package.version`이 source 정본이다.
