...
### 공개 호환성 기준 — 2026-09-23 사용자 결정

- Aigent Hive의 최초 공개 안정판은 `0.9.1`. 하위 호환성 보장과 과거 이력 참조의 시작점도 `0.9.1`
- `0.9.1` 이전 버전은 하위 호환성 보장·과거 이력 참조 대상에서 제외. 기존 기록이나 동결 자료의 삭제를 뜻하지 않는 범위 결정
- `0.9.1`부터 최신 버전까지 모든 버전의 하위 호환성 보장 필수. 바로 이전 버전이나 일부 대표 버전만으로 범위 축소 금지
- 앞으로 출시하는 모든 버전에도 같은 의무 적용. 버전 번호 증가만으로 이전 공개 버전의 호환성 의무 해제 금지
- 여기서 모든 공개 버전은 공개 안정판 전체를 의미. 사용자 확인에 따라 번호 공개 시험판은 이 전체 보장 범위에서 제외
- 이 결정은 지속적으로 지켜야 할 요구사항. 모든 과거 버전의 구현·검증 완료를 뜻하는 근거로 사용 금지

Hive는 pre-1.0에도 같은-major non-breaking 정책을 적용. `0.9.1` 이후 기존 supported contract 파괴 금지. Breaking change의 `0.y.0` minor 은폐 금지.
