...
| orchestration | host native 또는 사용자 선택 OMX/OMC가 소유 |
| runtime 관찰 | 사용자 선언 정본, opt-in public path/`--version` probe만 허용, foreign state read 금지 |
| 재구현 금지 | Hive plan·Ralph·team·provider session engine 없음 |
| Skill | 이름·source·revision·content digest·권한을 보여준 뒤 개별 수동 승인, RFC 8785 JCS consent digest |
...
| 라이선스 | CLI/source는 `Apache-2.0`, `harness/**`와 생성된 Hive 소유 material은 `Apache-2.0` |
