...
- `harness/**`를 제외한 저장소 source는 `Apache-2.0`로 배포한다.
...
- GitHub primary license 감지를 위해 root `LICENSE`는 수정하지 않은 Apache-2.0 전문만 포함하고 scope 설명은 `docs/licensing.md`로 분리한다.
- `harness/LICENSE`는 출하 source의 Apache-2.0 전문을 제공한다.
- Rust package metadata는 `Apache-2.0`를 선언한다.
...
- CLI/source의 Apache-2.0-covered 수정판을 배포하는 주체는 Apache-2.0 의무를 따라야 한다.
