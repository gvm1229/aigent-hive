...
공개 `0.10.0-test.6`의 선택형 벡터 기능 안내. `test.4`에는 벡터 기능 없음.
성능 수용과 세 운영체제 공개 설치 검증 완료. [수용 근거](../research/vector-public-test6-2026-08-29.md)·[활성 계획](../plans/active/hybrid-vector-search-0.10.0.md) 참조.

## 미리 알아둘 대기 시간

- 처음에는 모델 다운로드와 전체 벡터 계산에 수 분 이상 소요 가능. Windows 합성 5만 청크·100모음의 실제 전체 생성 약 10분 39초
- 이후에는 바뀐 내용 중심으로 재사용. 같은 시험의 100개 변경은 벡터 약 52초, FTS 포함 약 70초. 소규모 변경 시간으로 단순 환산 금지
- 의미 검색은 질문 변환·범위/원문 검증을 포함해 수 초 대기 가능. 같은 시험의 p95는 전역 약 3.07초·한 모음 약 2.28초
- 모델·처리 계약 변경 또는 벡터 삭제 뒤에는 전체 재계산 가능. 최초 한 번으로 영구 종료된다는 보장 없음
- 정확한 문구에는 기본 FTS 사용 가능. 기기·자료·모음 수에 따른 차이 존재, 안전 검사와 중단 재개는 유지
- [시각 설명 문서](../hive-vector-search.ko.html)에서 생성·검색 흐름과 기술 구성 확인 가능
...
- 실제 연결·영향 관계: 문서의 명시적 연결이나 관계 그래프 확인
  - 벡터 유사도만으로 인과관계·사실 확정 금지

기존 `knowledge-recall`이 질문에 맞는 경로를 선택. 새 Skill은 필요하지 않으며,
질문 응답을 위한 모델 자동 설치·새 색인 생성 금지.

## 갱신 뒤 사용 여부 선택

- 기존 설치의 다음 일반 Hive 작업: 기존 설정 재질문 없이 의미 검색 질문 1개
- 질문 안내: 표현이 달라도 뜻이 비슷한 지식 검색 사용 여부. 기존 검색 유지
- `예`: 사용 의사만 저장, 새 대화에 붙여넣을 설정 안내문 제공
  - 고정 대상: 사용자 전역 지식과 현재 연결된 shared 모음
  - 제외 대상: project-private·confidential·새로 발견한 모음
  - 새 대화: 미리보기의 다운로드·저장 위치·Python 조건·동의 지문 확인 뒤 설치·생성·재개·FTS 대체 경로 확인
- `아니요`: 사용 의사 `no` 저장, 명시적 벡터 설정 요청 전 자동 재질문·재권유 제외
- 무응답·취소: `no` 기록 없음. 현재 대화의 반복 질문 제외, 다음 대화의 재안내 대상
- 답변 파일: `.hive/config/user-feature-answers.yml`. 정상 갱신·보존형 재설치 뒤 유지, 원본 지식·FTS·기존 벡터 자료 삭제 없음
...
완료 전 새 색인 활성화 금지. 기밀 모음은 매번 별도의 현재 작업 승인 필요.
...
현재 프로젝트의 비공개 자료를 다른 프로젝트로 확장 검색 금지.
