...
> 공개 Stable: `0.10.3`
> 현재 단계: 협업자 설치 선택권 범위 추가, 구현·재검증 대기
> 이전 공개 수용: `0.11.0-test.1`; 다음 공개 시험: `0.11.0-test.2`
> 결정: [ADR-0023](../decisions/ADR-0023-foundation-refactor.md), [기존 ADR-0022](../decisions/ADR-0022-global-user-update.md)

## 현재 요청과 경계

- 현재 요청: `PDC-001–003` 생성기 개선·검증을 0.11.0 범위에 추가. 예시 프로젝트 읽기 전용, 제품 검증 후 해당 프로젝트 수정 계획 수립
...
현재 요청의 종료 조건: 1~5단계 구현·검증·수정 계획 완료. 실행 순서: `PDC-001–003` → 재개한 `RFR-001`·`0.11.0-test.2` 수용 → 예시 프로젝트 수정 계획. 예시 프로젝트 실제 변경은 현재 범위 밖.
