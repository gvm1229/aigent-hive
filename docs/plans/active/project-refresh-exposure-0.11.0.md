...
- `test.3` 공개·Windows 설치 완료. 실제 갱신의 기준본·경로 예약 오류 발견, 미공개 test.4 후보 취소. 두 수정은 `0.11.0-test.5` 검증, 안정판·main 변경 제외
...
- [ ] [PRF-002] 공개 시험판과 설치된 스킬의 실제 호스트 발견·자연어 요청 검증
  - state: agent-owned; depends: PRF-001,PRF-004,PRF-005
...
`PRF-001` → `PRF-004` → `PRF-005` → `PRF-002` → `PRF-003`. 제품 검증 전 실제 예시 갱신 금지.
