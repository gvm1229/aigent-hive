...
- [ ] [RF-P01] 현재 규칙→정본→코드 검사→진입 명령→시험의 대응표와 작업별 지침 읽기 비용 기록
- [ ] [RF-P02] 이미 존재하는 판정 함수 재사용과 빠진 직접 CLI 진입 검사 보강; 동일 실패의 일관된 기존 결과 코드 유지
- [ ] [RF-P03] 코드로 검증되는 상세 절차의 지침 중복 축소, 정본과 현재 세 호스트 투영 동기화
- [ ] [RF-P04] 허용·거부·취소·재개·일반 질문의 전후 행동 평가와 정보량·호출 수 비교
...
- `RF-P01`에서 고정한 일반 질문·지식 작업 지침 경로의 전체 읽기 바이트 20% 이상 축소 목표; 필수 행동 평가 전부 유지
...
- [ ] [RF-T01] `user_install.rs`와 `hive-render/src/lib.rs`의 책임별 내부 모듈 분리, 기존 진입 함수·출력·테스트 호환성 유지
- [ ] [RF-T02] 두 호출 경로의 공통 파일 교체·복구 원시 기능 추출, 순환 의존 없는 공유 경계와 오류 계약 검증
- [ ] [RF-T03] 프로젝트 생성과 사용자 설치의 순차 전환, 기존 변경 계획·지문·거부 결과·중단 후 복구의 동등성 비교
- [ ] [RF-T04] 프로젝트 갱신까지 공통 처리 연결, 장애 삽입·동시 변경·세 운영체제 수용과 기존 이관 검증
