...
- [x] `HBC95-001` shipped full project-base release registry 작성: migration source로 선언할 모든
  release의 exact rendered base·digest·required render input 수집. 우선 회귀 기준 `0.9.1`·`0.9.2`·`0.9.3`
- [x] `HBC95-002` registry 단일 소비 경로 연결: `historical_project_upgrade_candidate_in`와
  `authenticate_historical_base`의 version lookup·full-ledger 인증·diagnostic을 같은 registry로 통합
- [x] `HBC95-003` migration-table build/validation gate 추가: `check-project-base-coverage.py`가
  `0.9.1`–`0.9.4` exact tag base·coverage digest를 검증. full base 이전 source range: candidate build 실패
- [x] `HBC95-004` `0.9.2` synthetic consumer fixture 회귀 추가: compiled CLI의 `scan`·`dry-run`·`apply`·`validate`
  전체 수명주기, modified Hive-owned projection의 three-way merge, user·foreign byte 보존, rollback 검증
- [x] `HBC95-005` declared source range matrix 회귀 추가: compiled CLI의 `0.9.1`·`0.9.2`·`0.9.3`
  candidate authentication·apply validation, `0.9.4` exact frozen byte base, missing·tampered baseline의
  no-mutation 실패, local npm package executable 확인

## 명시 보류 배포 수용

`HBC95-006` public `0.9.2 → 0.9.5-test.N` project upgrade 수용은 현재 유지보수자 경계에 따라
...
- public test artifact와 stable candidate에서 같은 matrix evidence 보관은 공개 배포 재승인 뒤 수행

## 출시 경계

`HBC95-001`–`HBC95-005`는 source implementation·local verification 범위. `HBC95-006`의 public
test와 stable publication은 현재 명시 보류이며, 재승인 뒤 release authority·external evidence 필요. version field 수동 변경,
