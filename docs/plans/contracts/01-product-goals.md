...
Aigent Hive: 사용자가 이미 로그인한 Codex, Claude Code, Gemini Antigravity 위의 로컬 agent harness. Rust CLI의 프로젝트 setup, 지침 projection, Markdown memory, SQLite 검색 인덱스, 검증·update·Hive-native orchestration 상태 관리. 모델 실행과 subagent process는 host 소유. 신규 OMX·OMC execution dependency 없음.
...
4. Hive가 현재 host의 declarative envelope·receipt·cancel capability를 검증하고, 지원되는 exact host event와 별도 동의가 있을 때만 scoped lifecycle hook 검토
...
9. 일반 작업은 bounded knowledge retrieval 뒤 승인된 최소 Hive Skill 선택
10. Hive가 durable role/run/event 상태·logical scheduling·cancel·team·goal 판단 소유, host가 exact envelope의 model·subagent 실행 담당
...
- subscription 인증, model call과 billing은 host 소유.
...
- Hive-native iterative plan·logical scheduler·team·multi-goal state machine 구현 허용.
- Model runtime·provider session engine·direct model/subagent launcher 구현 금지.
- 신규 workflow의 OMX·OMC 선택·실행·fallback 금지. Legacy external run은 read-only provenance만 보존.
- Host capability 실패의 조용한 backend fallback 금지. `unsupported|dispatch-uncertain` 정직한 중지.
- Ambient session pointer authority 금지. Exact target·event head·control epoch·single-action authority 필수.
- Hook은 host가 exact event를 지원하고 capability·event·path·digest에 별도 동의가 있을 때만 설치·활성화. 거절은 완전히 지원되는 상태.
...
- provider API SDK
- 자체 model router
- 자체 model·subagent process launcher
...
- Ambient pointer·unscoped Stop continuation hook
