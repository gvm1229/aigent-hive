...
Aigent Hive는 사용자가 이미 로그인한 Codex, Claude Code, Gemini Antigravity 위에서 동작하는 로컬 agent harness. Rust CLI가 프로젝트 setup, 지침 projection, Markdown memory, SQLite 검색 인덱스, 검증 계약과 update를 관리. 모델 실행과 subagent process는 검증된 host-native capability가 기본 소유하며, OMX·OMC는 사용자의 명시 선택 또는 고정된 `0.8.x` 실행에만 호환 경로로 사용.
...
4. Hive가 현재 host의 native capability를 우선 resolve하고, 명시 선택 또는 고정된 `0.8.x` owner일 때만 compatible OMX/OMC를 검증하며, 지원되는 exact host event와 별도 동의가 있을 때만 선택적 integrity hook 검토
...
9. 일반 작업은 bounded knowledge retrieval 뒤 요청에 맞는 승인된 최소 Skill을 자동 선택하고 실행은 host-native capability에 위임
...
- 검증된 host-native capability가 기본이며 OMX·OMC는 명시 선택 또는 고정된 `0.8.x` 실행에만 사용.
...
- Provider session runtime·subagent scheduler 복제
