...
"""
    ...
    def write_user_setup_answers(
        ...
        *,
        ...
        if skills_mode == "all":
            ...
            skills = {
                "mode": "individual",
                "selected": selected_skills or ["user-setup"],
            }
        write_yaml(
            path,
            {
                ...
                "profile": {"contexts": ["web-developer"]},
                "persona": {"id": "balanced"},
            ...
            with self.subTest(host=host):
                ...
                if host == "antigravity":
                    self.assertTrue(
                        (
                            user_root
                            / ".gemini/config/skills/user-setup/SKILL.md"
                        ).is_file()
                    )
                    self.assertEqual(
                        json.loads(
    ...
    def test_global_setup_projects_selected_skills_and_wiki_preferences(self) -> None:
        ...
        self.assertIn(
            "knowledge-recall", preview_result["data"]["resolved_skills"]
        )
        self.assertIn(
        ...
        reduced = self.write_user_setup_answers(
            ...
            selected_skills=["user-setup", "prompt-refine"],
            usage_enabled=True,
            threshold=17,
        )
    ...
    def test_global_setup_preserves_all_host_guidance_markers(self) -> None:
        ...
        answers = self.write_user_setup_answers(
            ...
            skills_mode="individual",
            selected_skills=["knowledge-capture"],
        )
            ...
            with self.subTest(host=host):
                ...
                if host == "claude":
                    self.assertTrue(
                        (
                            target
                            / ".claude/skills/project-refresh/SKILL.md"
                        ).is_file()
                    )
                else:
    ...
    def test_upgrade_preserves_local_skill_and_recovers_injected_failure(self) -> None:
        ...
        self.assertEqual(
            reports[".agents/skills/quick-answer/SKILL.md"]["disposition"],
            "local-preserved",
        )
        self.assertTrue(
            reports[".agents/skills/quick-answer/SKILL.md"]["local_priority"]
        )
