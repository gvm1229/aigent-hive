...
- Treat every release request as implementation, verification, or a numbered public test by default. Stable `tag`, protected `main` integration, publication, and installation are prohibited unless the maintainer explicitly authorizes the named stable version in the current request. Do not infer that authority from “release”, “ship”, “continue”, or “all todos”.
- Abort a continued task only for an exact user-owned manual blocker, a required Codex restart, or completed task criteria. Treat every other error, unavailable host, failed test, stale reference, or partial evidence gap as continued agent-owned work.
...
- Reuse useful Hive-owned workflows from canonical product Skills under `harness/skills/` after scope, safety, consent, and conformance review. This source workspace has no separate tracked Skill inventory except an explicit maintainer-authorized, nonshipping source-project Skill under `.agents/skills/update-summary/`; source-only constraints otherwise belong in repository directives. Never import installed consumer state, runtime data, or user knowledge into source.
